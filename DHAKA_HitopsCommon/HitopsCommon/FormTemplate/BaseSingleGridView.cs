...
{
    public partial class BaseSingleGridView : BaseView
    {
        #region FIELD AREA *************************************
        ...
        {
            this.Load += new EventHandler(BaseSingleGridView_Load);
            return;
        }

        void BaseSingleGridView_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    // Set DragCell Helper
                    SelectedCellsBorderHelper selectedCellsHelper = new SelectedCellsBorderHelper(this.Grid);
                    new DragCellsValuesHelper(selectedCellsHelper);
                    
                    // Column Init
                    if (this.Grid.Columns == null || this.Grid.Columns.Count == 0) this.Grid.Columns.Clear();

                    // Default Grid Option
                    this.Grid.OptionsView.ShowGroupPanel = false;
                    
                    this.Grid.OptionsSelection.MultiSelect = true;
                    this.Grid.OptionsSelection.MultiSelectMode = GridMultiSelectMode.CellSelect;

                    this.Grid.FocusRectStyle = DrawFocusRectStyle.RowFocus;
                    this.Grid.IndicatorWidth = BaseGridCommon.getIndicatorDefaultWidth;
                    this.Grid.OptionsClipboard.CopyColumnHeaders = DevExpress.Utils.DefaultBoolean.False;
                    
                    // Default Event
                    this.Grid.RowCellStyle += new RowCellStyleEventHandler(this.gridView_RowCellStyle);

                    this.Grid.ShownEditor += new EventHandler(this.gridView_ShownEditor);
                    this.Grid.ShowingEditor += new CancelEventHandler(this.gridView_ShowingEditor);
                    this.Grid.CustomRowCellEdit += new CustomRowCellEditEventHandler(this.Grid_CustomRowCellEdit);

                    this.Grid.CustomDrawCell += new RowCellCustomDrawEventHandler(this.gridView_CustomDrawCell);
                    this.Grid.CustomDrawRowIndicator += new RowIndicatorCustomDrawEventHandler(this.gridView_CustomDrawRowIndicator);

                    this.Grid.KeyPress += new KeyPressEventHandler(gridView_KeyPress);
                    this.Grid.KeyDown += new KeyEventHandler(gridView_KeyDown);
                    
                    this.Grid.InitNewRow += new InitNewRowEventHandler(gridView_InitNewRow);
                    this.Grid.CellValueChanged += new CellValueChangedEventHandler(this.Grid_CellValueChanged);

                    this.Grid.GridControl.ProcessGridKey += new KeyEventHandler(this.GridControl_ProcessGridKey);
                    this.Grid.GridControl.DataSourceChanged += new EventHandler(this.GridControl_DataSourceChanged);

                    this.Grid.GotFocus += new EventHandler(this.Grid_GotFocus);
                    this.Grid.PopupMenuShowing += new PopupMenuShowingEventHandler(this.Grid_PopupMenuShowing);
                    this.Grid.Click += new EventHandler(this.Grid_Click);
                    this.Grid.MouseDown += new MouseEventHandler(this.Grid_MouseDown);

                    this.Grid.GridControl.KeyDown += new KeyEventHandler(this.GridControl_KeyDown);

                    this.Grid.ShownEditor += new EventHandler(this.Grid_ShownEditor);
                    this.Grid.RowCellClick += new RowCellClickEventHandler(this.Grid_RowCellClick);
                }
